...
class NeuralLayer:
    def __init__(self, n_inputs: int, n_outputs: int, activation: Activation):
        ...
        self.weights = np.multiply(np.random.randn(n_inputs, n_outputs), np.sqrt(2.0 / n_inputs), dtype=np.longdouble)
        self.biases = np.zeros((1, n_outputs))
        self.activation = activation
    def forward(self, inputs) -> None:
        self.inputs = np.array(inputs, dtype=np.longdouble)
    ...
    def backward(self, error, learning_rate):
        ...
        if error.ndim == 0:
            error = np.full(self.output.shape, error, dtype=np.longdouble)
        else:
            try:
                error = np.broadcast_to(error, self.output.shape).astype(np.longdouble, copy=False)
            except ValueError as exc:
                raise ValueError(f"Error shape {error.shape} cannot be broadcast to output shape {self.output.shape}") from exc
        delta = np.multiply(error, self.activation.derivative(self.output), dtype=np.longdouble)
        self.weights -= np.multiply(learning_rate, self.inputs.T.dot(delta), dtype=np.longdouble)
        self.biases -= np.multiply(learning_rate, np.sum(delta, axis=0, keepdims=True), dtype=np.longdouble)
...
class NeuralNetwork:
    ...
    def train(self, m: float = 1e-7) -> None:
