...
for a, i in zip(x, target):
    print(f"Inputs: {a} AI Output: {nn.forward(a)} Expected Output: {i} Predicted output: {np.random.normal(nn.forward(a)[0][0],stable_exp(nn.forward(a)[0][1]))}")