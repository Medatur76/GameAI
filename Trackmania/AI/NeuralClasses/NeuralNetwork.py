...
class NeuralNetwork(): 
    def __init__(self, n_inputs: int, n_layers: int, n_outputs: int, n_output_activations: list[Activation], base_activation: Activation=Activation):
        ...
        self.layers.append(NeuralLayer(n_inputs+(n_layers)*2, n_outputs, None, n_output_activations))
    def forward(self, inputs, use_final_activation=True):
        ...
        if use_final_activation:
            self.layers[len(self.layers)-1].forward(self.layers[len(self.layers)-2].output)
